  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsOpen(false);
        setSuccess(false);
        setFormData({ clientName: '', clientPhone: '', notes: '' });
      }, 3000);
    } catch (error) {
      console.error(error);
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200 relative z-10">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-slate-900 text-lg">Request a Quote</h3>
            ...
            <div className="p-6">
              {success ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500">The provider has been notified and will contact you shortly.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Details / Service Needed</label>
                    <textarea
                      rows={3}
